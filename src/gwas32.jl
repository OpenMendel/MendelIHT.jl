...
function iht(
	...
	b0       :: DenseVector{Float32} = SharedArray(Int, p, init = S -> S[localindexes(S)] = b[localindexes(S)], pids=pids), 
	Xb       :: DenseVector{Float32} = xb(x,b,IDX,k, means=means, invstds=invstds, pids=pids), 
	Xb0      :: DenseVector{Float32} = SharedArray(Int, p, init = S -> S[localindexes(S)] = Xb[localindexes(S)], pids=pids), 
...
function L0_reg(
	...
	for mm_iter = 1:max_iter
		...
		isinf(next_obj) && throw(error("Objective function is Inf3232, aborting..."))
...
#			output = Dict{ASCIIString, Any}("time" => -1.0, "loss" => -1.0, "iter" => -1, "beta" => fill!(b,Inf3232))
