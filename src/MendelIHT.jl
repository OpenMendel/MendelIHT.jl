...
function MendelIHT(control_file = ""; args...)
    ...
    elseif keyword["model_sizes"] != ""
        ...
        return L0_reg(v, snpmatrix, non_genetic_cov, phenotype, J, k, use_maf = maf_weights, glm = glm)
    end
    #
...
function iht!(
    ...
    elseif glm == "logistic"
        ...
        _iht_gradstep_logistic(v, x, z, y, mean_vec, std_vec, μ, J, k, temp_vec, storage)
    else
...
function L0_reg(
    ...
    for mm_iter = 1:max_iter
        ...
        println("current iteration is " * string(mm_iter) * " and loss is " * string(loss))
...
function L0_logistic_reg(
    ...
    SnpArrays.At_mul_B!(v.df, x, y, mean_vec, std_vec, store[1])
    BLAS.At_mul_B!(v.df2, z, y)
